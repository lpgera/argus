...
const log = Logger({ name: 'dirigera-light-sensor' })
...
const onTick = async () => {
  try {
    const lightSensor = await dirigeraClient.lightSensors.get({
      id: process.env.DIRIGERA_LIGHT_SENSOR_ID,
    })

    const measurements = [
      {
        type: 'illuminance',
        value: lightSensor.attributes.illuminance,
      },
    ]
...
const measurementJob = CronJob.from({
  cronTime: process.env.DIRIGERA_LIGHT_SENSOR_MEASUREMENT_CRON ?? '*/5 * * * *',
  onTick,
})
