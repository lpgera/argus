...
const log = Logger({ name: 'vallhorn' })
...
dirigeraClient.startListeningForUpdates(async (update) => {
  ...
  if (
    ...
    await client.storeMeasurement({
      type: 'motion',
      value: update.data.attributes.isDetected ? 1 : 0,
    })
  }
})
...
const onTick = async () => {
  try {
    const vallhorn = await dirigeraClient.motionSensors.get({
      id: process.env.DIRIGERA_VALLHORN_ID,
    })

    await client.storeMeasurement({
      type: 'motion',
      value: vallhorn.attributes.isDetected ? 1 : 0,
    })
  } catch (error) {
    log.error(error)
  }
...
const measurementJob = CronJob.from({
  cronTime: process.env.VALLHORN_MEASUREMENT_CRON ?? '*/5 * * * *',
  onTick,
})
